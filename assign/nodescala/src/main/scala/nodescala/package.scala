...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = fs match {
      case Nil => Future(Nil)
      case hd :: tl => hd.flatMap(x => all(tl).flatMap(y => Future(x :: y)))
    }
    ...
    def run()(f: CancellationToken => Future[Unit]): Subscription = {
      ...
      val tokenTargetFuture = f(token.cancellationToken)
      token
    }
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
     ...
     */
    def now: T = try{Await.result(f, 0 seconds)}catch{case _:Throwable => throw new NoSuchElementException}
     ...
     */
    def continueWith[S](cont: Future[T] => S): Future[S] = f.flatMap(x => Future(cont(f)))
     ...
     *  The resulting future contains a value returned by `cont`.
     */
    def continue[S](cont: Try[T] => S): Future[S] = f.flatMap(x => Future(cont(Success(x))))
