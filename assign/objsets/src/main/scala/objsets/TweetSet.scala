...
abstract class TweetSet {
  ...
  def compareRetweet(e: Tweet): Tweet
...
class Empty extends TweetSet {
  ...
  def mostRetweeted: Tweet = throw new java.util.NoSuchElementException("Cannot find tweet: Empty set")

  def compareRetweet(e: Tweet) = e
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def union(that: TweetSet): TweetSet =
    that match {
      case n: NonEmpty => left union (right union that.incl(elem))
      case _ => this
    }

  def mostRetweeted: Tweet = left compareRetweet (right compareRetweet elem)

  def compareRetweet(e: Tweet) ={
    val valueMostRetweeted = mostRetweeted // Essential for performance
    if (valueMostRetweeted.retweets > e.retweets) valueMostRetweeted else e
  }
